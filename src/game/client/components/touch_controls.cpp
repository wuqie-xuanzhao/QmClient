...
{
	return {CButtonLabel::EType::LOCALIZED, Localizable("Spectate mode")};
}
...
void CTouchControls::UpdateButtonsGame(const std::vector<IInput::CTouchFingerState> &vTouchFingerStates)
{
	...
	if(GotDirectFingerState)
	{
		...
		vec2 WorldScreenSize;
		Graphics()->CalcScreenParams(Graphics()->GameScreenAspect(), Zoom, &WorldScreenSize.x, &WorldScreenSize.y);
