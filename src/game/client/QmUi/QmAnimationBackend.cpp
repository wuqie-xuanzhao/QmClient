...
namespace qm_animation
{
	int NormalizeMotionLevel(const int MotionLevel)
	{
		return std::clamp(MotionLevel, 0, 2);
	}

	SUiAnimTransition ApplyMotionLevel(SUiAnimTransition Transition, const int MotionLevel)
	{
...
void CQmAnimationBackend::Reset()
{
	m_TimeSec = 0.0f;
...
float CQmAnimationBackend::ApplyEasing(float t, const SUiAnimTransition &Transition) const
{
...
float CQmAnimationBackend::ApplyTrackEasing(float t, const SActiveTrack &Track) const
{
	const float Clamped = std::clamp(t, 0.0f, 1.0f);
...
float CQmAnimationBackend::TrackProgress(const SActiveTrack &Track) const
{
...
bool CQmAnimationBackend::StartTrack(const STrackKey &Key, const SUiAnimRequest &Request, float StartValue, float StartVelocity)
{
	SActiveTrack Track;
...
void CQmAnimationBackend::StartQueuedTracks(const STrackKey &Key, float StartValue)
{
...
bool CQmAnimationBackend::StartTrackInterrupt(const STrackKey &Key, const SUiAnimRequest &Request, const SActiveTrack &Active)
{
	// 先复制当前状态再移除旧轨道，避免立即完成的接管请求把旧轨道遗留在表中。
	const float StartValue = Active.m_Current;
	...
	if(Takeover.m_TrackId != ActiveTrackId)
		CancelAwaitedTrack(ActiveTrackId);
	m_ActiveTracks.erase(Key);
	const bool Started = StartTrack(Key, Takeover, StartValue, StartVelocity);
	if(!Started)
		StartQueuedTracks(Key, Request.m_Target);
	return Started;
}

void CQmAnimationBackend::CompleteTrack(const STrackKey &Key, const SActiveTrack &Track)
{
...
void CQmAnimationBackend::CompleteAwaitedTrack(uint32_t TrackId)
{
...
void CQmAnimationBackend::CancelAwaitedTrack(uint32_t TrackId)
{
	auto ItTrackGroup = m_TrackAwaitGroups.find(TrackId);
	if(ItTrackGroup == m_TrackAwaitGroups.end())
		return;
...
void CQmAnimationBackend::CancelAwaitGroup(uint32_t GroupId)
{
...
void CQmAnimationBackend::CancelQueuedTracksForKey(const STrackKey &Key)
{
...
bool CQmAnimationBackend::IsTrackPending(uint32_t TrackId) const
{
	if(TrackId == 0)
		return false;
...
float CQmAnimationBackend::CurrentValueFor(const STrackKey &Key, float DefaultValue) const
{
...
void CQmAnimationBackend::SetValue(uint64_t NodeKey, EUiAnimProperty Property, float Value)
{
...
float CQmAnimationBackend::GetValue(uint64_t NodeKey, EUiAnimProperty Property, float DefaultValue) const
{
	const STrackKey Key{NodeKey, Property};
	return CurrentValueFor(Key, DefaultValue);
}

bool CQmAnimationBackend::RequestAnimation(const SUiAnimRequest &Request)
{
	SUiAnimRequest EffectiveRequest = Request;
	if(Request.m_Transition.m_RespectMotionLevel)
		EffectiveRequest.m_Transition = qm_animation::ApplyMotionLevel(Request.m_Transition, g_Config.m_QmUiMotionLevel);
...
void CQmAnimationBackend::AdvanceSpring(SActiveTrack &Track, float Dt) const
{
	const float Delay = std::max(0.0f, Track.m_Transition.m_DelaySec);
...
void CQmAnimationBackend::Advance(float Dt)
{
...
bool CQmAnimationBackend::HasActiveAnimation(uint64_t NodeKey, EUiAnimProperty Property) const
{
	return m_ActiveTracks.contains({NodeKey, Property});
}

void CQmAnimationBackend::PruneResolveTargetCache(uint64_t CurrentUseCounter)
{
...
float CQmAnimationBackend::ResolveTargetValue(uint64_t NodeKey, EUiAnimProperty Property, float Target, const SUiAnimTransition &Transition)
{
...
void CQmAnimationBackend::PruneColorTargetCache(uint64_t CurrentUseCounter)
{
...
ColorRGBA CQmAnimationBackend::ResolveColorFromValue(uint64_t NodeKey, const ColorRGBA &Current, const ColorRGBA &Target)
{
...
float CQmAnimationBackend::ResolveColorMixValue(uint64_t NodeKey, const ColorRGBA &Target, float DurationSec, EEasing Easing)
{
...
int CQmAnimationBackend::ActiveTrackCount() const
{
	return static_cast<int>(m_ActiveTracks.size());
}

int CQmAnimationBackend::QueuedTrackCount() const
{
	int Count = 0;
	for(const auto &Pair : m_QueuedTracks)
...
void CQmAnimationBackend::RegisterCustomEasing(uint32_t EasingId, FCustomEasing pfnEasing, void *pUser)
{
...
void CQmAnimationBackend::UnregisterCustomEasing(uint32_t EasingId)
{
	m_CustomEasings.erase(EasingId);
}

bool CQmAnimationBackend::PollCompletedEvent(SUiAnimCompleteEvent &EventOut)
{
...
uint32_t CQmAnimationBackend::AwaitTracks(const uint32_t *pTrackIds, int NumTrackIds)
{
...
bool CQmAnimationBackend::PollGroupCompletedEvent(SUiAnimGroupCompleteEvent &EventOut)
{
...
float CQmAnimationBackend::TimeSec() const
{
	return m_TimeSec;
}
