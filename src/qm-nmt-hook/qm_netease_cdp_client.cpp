...
namespace QmNeteaseCdp
{
	...
	{
		return L"/api/song/lyric?id=" + std::to_wstring(SongId) + L"&lv=-1&kv=-1&tv=-1";
	}

	bool FetchNeteaseLyricEndpoint(uint64_t SongId, NeteaseLyrics::SRawLyrics *pOut, int TimeoutMs)
	{
		if(pOut == nullptr || SongId == 0)
			return false;
		pOut->m_Lrc.clear();
		pOut->m_Yrc.clear();
#if defined(_WIN32)
		...
		if(hSession == nullptr)
			return false;
		const DWORD Timeout = (DWORD)std::clamp(TimeoutMs, 250, 5000);
		WinHttpSetTimeouts(hSession, Timeout, Timeout, Timeout, Timeout);
		HINTERNET hConnect = WinHttpConnect(hSession, L"music.163.com", INTERNET_DEFAULT_HTTPS_PORT, 0);
		const std::wstring RequestPath = BuildNeteaseLyricRequestPath(SongId);
		HINTERNET hRequest = hConnect != nullptr ? WinHttpOpenRequest(hConnect, L"GET", RequestPath.c_str(), nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE) : nullptr;
		bool Success = false;
		std::string Body;
		if(hRequest != nullptr)
		{
			// 只接受网易云自身的固定 endpoint，禁止跟随重定向到其它 host。
			DWORD RedirectPolicy = WINHTTP_OPTION_REDIRECT_POLICY_NEVER;
		...
		if(hRequest != nullptr && WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0) &&
			...
			WinHttpQueryHeaders(hRequest, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER, WINHTTP_HEADER_NAME_BY_INDEX, &Status, &StatusSize, WINHTTP_NO_HEADER_INDEX) && Status == 200)
		{
			std::array<char, 8192> Buffer{};
			for(;;)
			{
		...
		if(Body.empty())
			return false;
		json_value *pRoot = json_parse(Body.data(), Body.size());
		...
		auto ReadLyric = [](const json_value *pObject, const char *pName) {
			if(pObject == nullptr || pObject->type != json_object)
				return std::string{};
			for(unsigned int Index = 0; Index < pObject->u.object.length; ++Index)
			{
				const auto &Entry = pObject->u.object.values[Index];
				if(Entry.name != nullptr && std::string_view(Entry.name, Entry.name_length) == pName && Entry.value != nullptr && Entry.value->type == json_object)
				{
					for(unsigned int Child = 0; Child < Entry.value->u.object.length; ++Child)
					{
						const auto &Field = Entry.value->u.object.values[Child];
						if(Field.name != nullptr && std::string_view(Field.name, Field.name_length) == "lyric" && Field.value != nullptr && Field.value->type == json_string)
							return std::string(Field.value->u.string.ptr, Field.value->u.string.length);
					}
				}
			}
			return std::string{};
		};
		pOut->m_Yrc = ReadLyric(pRoot, "yrc");
		pOut->m_Lrc = ReadLyric(pRoot, "lrc");
		json_value_free(pRoot);
		Success = !pOut->m_Yrc.empty() || !pOut->m_Lrc.empty();
		return Success;
#else
		(void)TimeoutMs;
		return false;
