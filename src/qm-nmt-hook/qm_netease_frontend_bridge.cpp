...
namespace QmNeteaseBridge
{
		...
		bool PositionAnchored = false;
		{
			...
			else
			{
				...
				if(ParsedLyrics)
				{
					...
					ClearLyricsLocked();
				// malformed Frontend 报告不能清掉仍然有效的 InternalApi/GDI
				// 结果；等待下一次合法报告或 endpoint 重试。
				NeedPublish = true;
			}
		}
		if(NeedPublish)
	...
	void CFrontendLyricBridge::Tick()
	{
		...
		const bool WorkerConnected = m_Worker.IsConnected();
		{
			...
			if(m_Timeline.m_vLines.empty() && Now - m_LastEndpointTick >= 2000)
			{
				m_LastEndpointTick = Now;
				NeedEndpoint = true;
				EndpointSongId = m_SongId;
			}
			if(m_PositionValid)
			{
		...
		{
			NeteaseLyrics::SRawLyrics Raw;
			if(QmNeteaseCdp::FetchNeteaseLyricEndpoint(EndpointSongId, &Raw))
			{
				NeteaseLyrics::STimeline Timeline;
				if(NeteaseLyrics::ParseRawLyrics(Raw, &Timeline))
				{
					std::scoped_lock Lock(m_Mutex);
					if(m_SongId == EndpointSongId && m_Source != QmNeteaseHook::ENeteaseLyricSource::Frontend)
					{
						m_Timeline = std::move(Timeline);
						m_Source = QmNeteaseHook::ENeteaseLyricSource::InternalApi;
						m_CurrentLyric.clear();
						m_LineStartMs = -1;
						m_LineEndMs = -1;
					}
	...
	void CFrontendLyricBridge::Publish(bool Force, bool PositionAnchored)
	{
		...
		bool HighPriorityTimelineActive = false;
		{
			...
			HighPriorityTimelineActive = m_Timeline.m_HasTiming && !m_Timeline.m_vLines.empty() &&
						     (m_Source == QmNeteaseHook::ENeteaseLyricSource::Frontend || m_Source == QmNeteaseHook::ENeteaseLyricSource::InternalApi);
