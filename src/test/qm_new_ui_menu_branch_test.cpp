...
TEST(QmNewUiMenuBranches, WeaponReloadAnimationHasIndependentToggleAndProbability)
{
	const std::string ConfigSource = ReadTextFile("src/engine/shared/config_variables_qmclient.h");
	const std::string MenusSource = ReadTextFile("src/game/client/components/qmclient/menus_qmclient.cpp");
	const std::string PlayersSource = ReadTextFile("src/game/client/components/players.cpp");

	EXPECT_NE(ConfigSource.find("MACRO_CONFIG_INT(QmWeaponReloadAnim, qm_weapon_reload_anim, 0, 0, 1"), std::string::npos);
	EXPECT_NE(ConfigSource.find("MACRO_CONFIG_INT(QmWeaponReloadAnimProbability, qm_weapon_reload_anim_probability, 100, 0, 100"), std::string::npos);
	EXPECT_NE(MenusSource.find("DoQmSettingsCheckboxAuto(&g_Config.m_QmWeaponReloadAnim"), std::string::npos);
	EXPECT_NE(MenusSource.find("RenderSliderWithValueInput(&s_QmWeaponReloadAnimProbabilityInputId"), std::string::npos);
	EXPECT_NE(MenusSource.find("if(g_Config.m_QmWeaponSwitchAnim || g_Config.m_QmWeaponReloadAnim)"), std::string::npos);
	EXPECT_NE(PlayersSource.find("g_Config.m_QmWeaponReloadAnim && ShouldRenderWeaponAnimation(ClientId)"), std::string::npos);
}
