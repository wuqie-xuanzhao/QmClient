...
void CServer::ProcessClientPacket(CNetChunk *pPacket)
{
	...
	if(Sys)
	{
		...
		else if(Msg == NETMSG_RCON_AUTH)
		{
			...
			if(!pName[0])
			{
				KeySlot = m_AuthManager.DefaultKey(RoleName::ADMIN);
				if(m_AuthManager.CheckKey(KeySlot, pPw))
					AuthLevel = AUTHED_ADMIN;
				else
				{
					KeySlot = m_AuthManager.DefaultKey(RoleName::MODERATOR);
					if(m_AuthManager.CheckKey(KeySlot, pPw))
						AuthLevel = AUTHED_MOD;
					else
					{
						KeySlot = m_AuthManager.DefaultKey(RoleName::HELPER);
						if(m_AuthManager.CheckKey(KeySlot, pPw))
...
std::optional<bool> CServer::RateLimitServerInfoConnless()
{
	...
	if(Config()->m_SvServerInfoRepliesPerSecond != 0 &&
		...
		return std::nullopt;
	return Config()->m_SvServerInfoPerSecond == 0 || m_ServerInfoNumRequests <= Config()->m_SvServerInfoPerSecond;
}
...
#define RESET() \
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_aClients[i].IncludedInServerInfo())
		{
			...
			int Score;
			const auto ClientScore = m_aClients[i].m_Score;
			if(ClientScore.has_value())
			{
				Score = ClientScore.value();
...
void CServer::CacheServerInfoSixup(CCache *pCache, bool SendClients, int MaxConsideredClients)
{
	...
	Packer.AddInt(std::min<int>(LEGACY_MAX_CLIENTS, maximum(MaxClients - maximum(Config()->m_SvSpectatorSlots, Config()->m_SvReservedSlots), PlayerCount))); // max players
	Packer.AddInt(ClientCount); // num clients
	Packer.AddInt(std::min<int>(LEGACY_MAX_CLIENTS, maximum(MaxClients - Config()->m_SvReservedSlots, ClientCount))); // max clients
...
void CServer::SaveDemo(int ClientId, float Time)
{
	if(IsRecording(ClientId))
	{
		...
		str_format(aNewFilename, sizeof(aNewFilename), "demos/%s_%s_%05.2f.demo", GetMapName(), aPlayerName, Time);
...
void CServer::ConAddSqlServer(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "7 to 9 arguments are required");
		return;
	}
