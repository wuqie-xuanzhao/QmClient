...
class Record(namedtuple("Record", "name time checkpoints")):
	...
	def unparse(self):
		return "\n".join([
			self.name,
			str(self.time),
			" ".join([str(cp) for cp in self.checkpoints] + [""]),
			"",
		])
...
def main():
	p = argparse.ArgumentParser(
		description="Merge multiple DDNet race database files",
	...
	p.add_argument(
		...
		metavar="IN",
		nargs="+",
		help="Text score databases to import; must have the format MAPNAME_record.dtb",
	)
	p.add_argument(
		"--dry-run",
		"-n",
		action="store_true",
		help="Don't write out the resulting SQLite database",
	)
	p.add_argument(
		"--stats",
		action="store_true",
		help="Display some stats at the end of the import process",
	)
	args = p.parse_args()
		...
		c.executemany(
			...
			[
				(
					...
					float(r.time),
					"TEXT",
					*[float(c) for c in r.checkpoints],
					None,
					False,
				)
				for map, record in records.items()
				for r in record
			],
		)
		conn.commit()
	...
	if args.stats:
		...
		print(
			f"Number of imported ranks: {sum(len(r) for r in records.values())}",
			file=sys.stderr,
		)
