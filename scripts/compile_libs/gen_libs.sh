...
function build_cmake_lib() {
	...
	if [ ! -d "${library_dir}" ]; then
		...
		if ! git -C "${library_dir}" rev-parse --is-inside-work-tree > /dev/null 2>&1; then
			log_error "ERROR: Existing library folder is not a Git checkout: ${library_dir}"
			exit 1
		fi
		if ! expected_commit="$(git -C "${library_dir}" rev-parse "${clone_target_reference}^{commit}" 2> /dev/null)"; then
			log_error "ERROR: Existing ${library_dir} checkout does not contain ${clone_target_reference}; use a clean build folder"
			exit 1
		fi
		actual_commit="$(git -C "${library_dir}" rev-parse HEAD)"
		if [[ "${actual_commit}" != "${expected_commit}" ]]; then
			log_error "ERROR: Existing ${library_dir} checkout is at ${actual_commit}, expected ${expected_commit}; use a clean build folder"
			exit 1
		fi
	fi
	(
...
function build_sqlite3() {
	if [ ! -d "sqlite3" ]; then
		...
		local sqlite_archive_url="https://www.sqlite.org/2024/${sqlite_archive_filename}"
		if command -v curl > /dev/null 2>&1; then
			curl --fail --location --silent --show-error --output "${sqlite_archive_filename}" "${sqlite_archive_url}"
		elif command -v wget > /dev/null 2>&1; then
			wget --no-verbose --output-document "${sqlite_archive_filename}" "${sqlite_archive_url}"
		else
			log_error "ERROR: Downloading sqlite3 requires curl or wget."
			exit 1
		fi
