...
def auth_add_p_line(username, level, pwhash, salt):
	if level not in ("admin", "mod", "moderator", "helper"):
		print(
			f"Warning: level ({level}) not one of admin, mod or helper.",
			file=sys.stderr,
		)
	if repr(level) != f"'{level}'":
		print(
			f"Warning: level ({level}) contains weird symbols, config line is possibly malformed.",
			file=sys.stderr,
		)
	if repr(username) != f"'{username}'":
		print(
			f"Warning: username ({username}) contains weird symbols, config line is possibly malformed.",
			file=sys.stderr,
		)
...
def main():
	...
	p.add_argument(
		...
		"-n",
		nargs=3,
		metavar=("USERNAME", "LEVEL", "PASSWORD"),
		action="append",
		default=[],
		help="username, level and password of the new user",
	)
