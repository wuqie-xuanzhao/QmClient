...
class Log(namedtuple("Log", "timestamp level line")):
    ...
    def parse(cls, line):
        if line.startswith("=="):
            ...
            return cls(
                datetime.removeprefix("["),
                level,
                line.removeprefix(" ").replace("]", ":", 1),
            )
...
class TestRunner:
    ...
    def run_tests(self, tests):
        ...
        duration = time() - start
        print(
            f"test result: {result}. {num_passed} passed; {num_skipped} skipped, {len(failed)} failed; finished in {duration:.2f}s"
        )
        print()
        return bool(failed)
...
class TestEnvironment:
    def __init__(self, runner, name, tmp_dir, timeout):
        ...
        with open(
            os.path.join(self.tmp_dir, "storage.cfg"), "w", encoding="utf-8"
        ) as f:
            f.write(f"""\
add_path .
...
""")
        ...
        self.ddnet_mastersrv = (
            os.path.relpath(runner.ddnet_mastersrv, self.tmp_dir)
            if runner.ddnet_mastersrv is not None
            else None
        )
        ...
        if self.runner.valgrind_memcheck:
            self.run_prefix_args = [
                ...
                "--suppressions={}".format(
                    relpath(
                        os.path.join(runner.repo_dir, "memcheck.supp"), self.tmp_dir
                    )
                ),
                "--track-origins=yes",
            ]
    ...
    def check_valgrind_memcheck_errors(self):
        if any(
            any(
                "== ERROR SUMMARY: " in line and "== ERROR SUMMARY: 0" not in line
                for line in stderr
            )
            for stderr in self.full_stderrs
        ):
            return "\n".join(
                line
                for stderr in self.full_stderrs
    ...
    def thread():
        ...
        for line in file:
            if output_file is None:
                ...
                output_file = open(
                    output_filename, "w", buffering=1, encoding="utf-8"
                )  # line buffering
...
class Runnable:
    ...
    def __init__(
        ...
        *,
        extra_env_vars={},
        log_is_stderr=False,
        allow_unclean_exit=False,
    ):
        self.name = name
        cur_env_vars = dict(os.environ)
        intersection = set(cur_env_vars) & (
            set(test_env.runner.extra_env_vars) | set(extra_env_vars)
        )
        if intersection:
            # pylint: disable=consider-using-f-string
            raise ValueError(
                "conflicting environment variable(s): {}".format(
                    ", ".join(sorted(intersection))
                )
            )
        new_env_vars = {
            **cur_env_vars,
            **test_env.runner.extra_env_vars,
            **extra_env_vars,
        }
        self.process = popen(
        ...
        stdout_wrapper = io.TextIOWrapper(
            self.process.stdout, encoding="utf-8", newline="\n"
        )
        stderr_wrapper = io.TextIOWrapper(
            self.process.stderr, encoding="utf-8", newline="\n"
        )
        self.full_stdout = []
        self.full_stderr = []
        ...
        run_timeout_thread(
            f"{global_name}_timeout", test_env, self.timeout_queue, self.events
        )
        run_lines_thread(
            f"{global_name}_stdout",
            stdout_wrapper,
            stdout_path,
            self.full_stdout,
            self.events if not log_is_stderr else None,
        )
        run_lines_thread(
            f"{global_name}_stderr",
            stderr_wrapper,
            stderr_path,
            self.full_stderr,
            self.events if log_is_stderr else None,
        )
        run_exit_thread(
...
class Mastersrv(Runnable):
    ...
    def next_event(self, timeout_id):
        ...
        if isinstance(event, Log):
            if event.line.startswith("warp::server: listening on http://[::]:"):
                self.port = int(
                    event.line[len("warp::server: listening on http://[::]:") :]
    ...
    def wait_for_startup(self, timeout=5):
        self.wait_for_log_prefix(
            "warp::server: listening on http://[::]:", timeout=timeout
        )

    def servers_json(self):
        return json.loads(
            urlopen(f"http://[::1]:{self.port}/ddnet/15/test-servers.json").read()
        )
...
def wait_for_kcp_status_line(server, predicate, timeout=5):
    ...
    raise AssertionError(
        f"did not find expected kcp_status line. Last status: {last_status}"
    )


def wait_for_transport(server, transport, timeout=5):
    return wait_for_kcp_status_line(
        server, lambda line: f"transport={transport}" in line, timeout=timeout
    )
...
def client_negotiates_kcp_transport(test_env):
    ...
    if (
        "kcp_capable=1" not in status
        or "kcp_negotiated=1" not in status
        or "session_count=1" not in status
    ):
...
def kcp_mixed_clients_after_fallback(test_env):
    ...
    wait_for_kcp_status_line(
        server, lambda line: "id=0" in line and "transport=legacy" in line, timeout=5
    )
    wait_for_kcp_status_line(
        server, lambda line: "id=1" in line and "transport=kcp" in line, timeout=5
    )
    server.exit()
...
def kcp_stress_many_clients(test_env):
    ...
    for ClientIndex in range(4):
        client = test_env.client(
            [
                f"player_name stress_kcp_{ClientIndex}",
    ...
    for ClientIndex in range(len(clients)):
        wait_for_kcp_status_line(
            server,
            lambda line, index=ClientIndex: (
                f"id={index}" in line and "transport=kcp" in line
            ),
            timeout=10,
        )
    server.command("kcp_status")
...
def smoke_test(test_env):
    ...
    game_uuid = str(
        UUID(
            server.teehistorian_filename.removeprefix("teehistorian/").removesuffix(
                ".teehistorian"
            )
        )
    )
    ...
    if not all(
        any(word in line for line in client1.full_stdout)
        for word in "cmdlist pause rank points".split()
    ):
...
def server_can_register(test_env):
    ...
    if (
        ...
        or servers_json["servers"][0]["info"]["map"]["name"] != "Tutorial"
...
def server_can_register_protocol(
    test_env, protocol_config, protocol_log, protocol_scheme
):
    mastersrv = test_env.mastersrv()
    wait_for_startup([mastersrv])
    server = test_env.server(
    ...
    wait_for_startup([server])
    server.wait_for_log_exact(
        f"register/{protocol_log}: successfully registered", timeout=5
    )
    servers_json = mastersrv.servers_json()
    if (
        len(servers_json["servers"]) != 1
        ...
        or not servers_json["servers"][0]["addresses"][0].startswith(
            f"{protocol_scheme}://[::1]:"
        )
    ):
        raise AssertionError(f"unexpected servers.json\n{servers_json}")
    server.exit()
    mastersrv.wait_for_log_prefix(
        f"mastersrv: successfully removed {protocol_scheme}://[::1]:", timeout=5
    )
    servers_json = mastersrv.servers_json()
    if len(servers_json["servers"]) != 0:
        raise AssertionError(f"unexpected servers.json\n{servers_json}")
...
def main():
    ...
    parser.add_argument(
        "--keep-tmpdirs",
        action="store_true",
        help="keep temporary directories used for the tests",
    )
    parser.add_argument(
        "--test-mastersrv", action="store_true", help="enforce testing of mastersrv"
    )
    parser.add_argument(
        "--timeout-multiplier",
        type=float,
        default=1,
        help="multiply all timeouts by this value",
    )
    parser.add_argument(
        "--valgrind-memcheck",
        action="store_true",
        help="use valgrind's memcheck on client and server",
    )
    parser.add_argument(
        "builddir", metavar="BUILDDIR", help="path to ddnet build directory"
    )
    ...
    if not os.path.exists(ddnet_mastersrv):
        if args.test_mastersrv:
            raise RuntimeError(
                f"mastersrv binary {ddnet_mastersrv!r} not found, compile it from src/mastersrv"
            )
        else:
