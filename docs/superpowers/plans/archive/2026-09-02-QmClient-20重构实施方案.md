...
> 2026-09-08：用户明确放弃本方案，继续在主项目 dyl_dev 开发。以下内容仅保留为历史参考，不再作为执行依据；重构分支及已保存改动保留。
...
- 当前工作树 `HEAD` 为 `b05fa3330`，分支为 `dyl_dev`。
- 当前 `src/game/version.h` 仍声明 `DDNET_VERSION_NUMBER 19090`、`GAME_RELEASE_VERSION_INTERNAL 19.9`，QmClient 版本为 `3.0.1`（`src/game/version.h:8-25`）。
- 仓库已有一份 20.0 上游同步台账，记录官方 20.0 tag 为 `a5a61806e434`，并明确 `dyl_dev` 已混入选择性同步和 QmClient 私有改动（`docs/superpowers/plans/2026-08-30-DDNet20上游同步实施计划.md:12-22`）。
- 当前工作树有未提交修改，不能用清理、回退或强制切换来制造“干净基线”。新 worktree 应从官方提交创建。
...
当前 `CGameClient` 直接 include QmClient/TClient 头文件（`src/game/client/gameclient.h:73-115`），并直接持有 `CQmClient`、`CTClient` 及大量相关组件成员（`src/game/client/gameclient.h:289-317`）。

启动时，`CGameClient::OnConsoleInit` 使用 `AddComponent` 将所有组件逐一放入同一个列表，QmClient 和 TClient 组件也在其中（`src/game/client/gameclient.cpp:549-637`）。之后所有组件统一经过 `OnInit`（`src/game/client/gameclient.cpp:940-946`）、`OnUpdate`、渲染和消息分发流程；输入则使用另一套有明确优先级的 `m_vpInput` 列表（`src/game/client/gameclient.cpp:642-659`）。
...
`qmclient` 与 `tclient` 源码目前被直接列入根 `CMakeLists.txt` 的 game-client 源文件列表（`CMakeLists.txt:3201-3361`），配置头也在通用 engine shared 源列表中（`CMakeLists.txt:2684-2689`）。歌词和网易云 hook 还拥有独立的 helper / DLL 目标（`CMakeLists.txt:188-261`）。
...
- 确认官方 game-client target 的源文件变量后，在根 `CMakeLists.txt` 只保留一个 QmClient manifest include 点。
- manifest 按 `core`、`features`、`services`、`ui`、`platform` 分组列出源文件；不使用无约束 `GLOB_RECURSE` 隐藏新文件。
...
frame_time_p95_ms
frame_time_p99_ms
fps_1pct_low = 1000 / frame_time_p99_ms
```

这里的 `fps_1pct_low` 是最差 1% 帧的近似倒数，不是平均 FPS，也不是任意采样点的最低 FPS。日志同时输出样本数、窗口长度、是否包含加载/切后台/窗口 resize，以免不同场景的数值被误比较。
