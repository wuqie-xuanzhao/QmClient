...
- 2026-08-26 Metal ON 回归中 `QuadContainerExBindsFragmentUniforms` 的 one-time-use/shared 顶点夹具与额外清屏会稳定产生黑帧，不能用于验证 fragment uniform。该测试恢复为 static private buffer 的 blit 上传路径，并由首次 render pass 清屏；连续三次运行通过，避免把资源策略差异误报为 uniform 绑定回归。
- 2026-08-26 并行编码评估补充：原生 Metal 当前由 `CGraphicsBackend_Threaded` 的单一 processor/graphics thread 顺序驱动；`gfx_render_thread_count` 只被 Vulkan secondary-command-buffer worker 路径消费，不能据此推导 Metal 必须单线程，也不能直接复用 Vulkan 的 `m_ThreadIndex`。Metal 预初始化日志现在记录 `requested_threads` 与 `actual_threads=1`，每 120 帧诊断样本增加 graphics-thread command processing 总时长/最大值。只有在这些样本或 GPU capture 证明 CPU encode 为主要瓶颈后，才进入独立的 Metal command partition/`MTLParallelRenderCommandEncoder` 设计；在此之前保持顺序编码以保护 drawable、encoder、stream buffer、render-target 和资源状态的顺序语义。
- 2026-08-26 用户报告闪烁、移动抖动和进入游戏前卡顿后，完成三项独立于 Vulkan 的 Metal 生命周期修复：中间非 present submit 保持 frame slot 为 in-flight 直到 GPU completion，避免 stream buffer 被 GPU 读取时复用；`nextDrawable == nil` 改为本帧跳过呈现而非 unsupported-command fatal；普通 Swap 不再保留 presented drawable 供延迟读回，只在录像实际启用时于当前提交编码 readback。新增 frame-state、contract 与 runtime 覆盖，并补充 drawable 获取最大值、16/33/100 ms 阈值和 unavailable 计数。旧自动诊断确认实际为 unlimited + VSync off，但其 `2026-08-27` 时间戳相对本轮环境日期 `2026-08-26` 超前，不能作为事件顺序证据。
- 2026-08-26 自动诊断稳定样本仍显示每 120 帧创建约 1 万个以上 `MTLBuffer`，而 frame-slot wait 为零。`CMD_RECREATE_BUFFER_OBJECT` 现在仅在目标槽位是等大小、非 one-time/private buffer 时走现有 blit 更新路径；one-time/shared buffer、大小变化和空槽仍销毁后创建，避免 CPU 可见 shared 内存与 in-flight GPU 读取竞争。诊断新增 `buffer_reuse_count`、`buffer_reuse_bytes`，用于下轮与同场景基线对照。
- 2026-08-26 Apple 文档与 `Starting a Game Port with Metal` macOS C++ sample 对照完成。当前 P0-P4 主命令路径已实现；P5 的 GPU timing、frame pacing、framebuffer-only/internal present texture、binary archive、Apple Silicon/Intel/外接屏长期矩阵尚未完成，Metal 继续保持 opt-in。`displaySyncEnabled` 在无限制帧率下没有 Metal 专用 pacing，且 `framebufferOnly = NO` 仍是性能候选，后续必须以本轮新诊断和 GPU capture 验证后再动，不能套用 MoltenVK 线程策略。
- 2026-08-26 官方 DDNet Vulkan 对照：其 `GetPresentationMode()` 在 `gfx_vsync=1` 选择 FIFO、在 `gfx_vsync=0` 选择 IMMEDIATE，并在 VSync 改变时重建 swapchain；该行为是 Vulkan swapchain 的后端策略，不能直接移植为 Metal 的线程或 drawable 生命周期实现。Metal 暂不改变 `CAMetalLayer.displaySyncEnabled` 语义；用户反馈的“VSync 已关闭仍在鼠标操作时横向闪烁”不能先归因于普通显示撕裂，需优先验证资源更新、命令缓冲和 drawable 内容一致性。
- 2026-08-26 缩小视野黑屏定位为索引缓冲容量不足：Metal 原先固定生成 `MAX_VERTICES / 4` 个 quad 的 `uint16` 索引，`CMD_INDICES_REQUIRED_NUM_NOTIFY` 只记录数量而不扩容，导致大 tile 批次被边界检查拒绝。现按通知数量重建索引缓冲，统一使用 `uint32` 索引与偏移，并在非 6 的倍数或分配失败时传播资源错误。`CMD_VSYNC` 仍只把显式配置映射到 `CAMetalLayer.displaySyncEnabled`，代码没有强制开启 VSync；100 Hz 现象需结合新诊断区分应用层节流、drawable 获取等待和 macOS compositor 限制。
- 2026-08-26 进一步核对主循环后确认：`gfx_vsync=0` 且 `gfx_refresh_rate=0` 时，`CGameClient::RenderThrottleRefreshRate()` 原本无条件转发菜单空闲刷新率，在线游戏因此被错误限制到显示器刷新率（本机为 100 Hz）。现仅在菜单激活时启用该空闲节流，游戏场景恢复真正的无限制渲染；新增 `QmMonitoringHelpers` 回归断言。
