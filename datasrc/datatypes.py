...
class NetArray(NetVariable):
    ...
    def emit_dump(self, offset):
        ...
        for i in range(0, self.size):
            ...
            result += [
                f'dbg_msg("snapshot", "%s\\t{self.base_name}[{int(i)}]=%d", aRawData, pObj->{self.base_name}[{int(i)}]);'
            ]
        return result
...
class NetTwIntString(NetArray):
    def __init__(self, name, size_chars):
        if size_chars % 4 != 0:
            raise ValueError(
                f"NetTwIntString '{name}' size must be divisible by 4 but is {size_chars}"
            )
    ...
    def emit_dump(self, offset):
        ...
        for i in range(0, self.size):
            ...
            result += NetVariable(self.var).emit_dump(offset + i)
            result += [
                f'dbg_msg("snapshot", "%s\\t{self.base_name}[{int(i)}]=%d\\tIntToStr: %s", aRawData, pObj->{self.base_name}[{int(i)}], aStr);'
            ]
        return result
