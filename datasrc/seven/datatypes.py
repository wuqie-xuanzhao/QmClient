...
class NetVariable:
    ...
    def emit_dump(self, offset):
        return [
            f'str_format(aRawData, sizeof(aRawData), "\\t\\t%3d %12d\\t%08x", {offset}, ((const int *)pData)[{offset}], ((const int *)pData)[{offset}]);'
        ]
...
class NetStringStrict(NetVariable):
    ...
    def emit_unpack(self):
        return [
            f"pMsg->{self.name} = pUnpacker->GetString(CUnpacker::SANITIZE_CC|CUnpacker::SKIP_START_WHITESPACES);"
        ]
...
class NetIntRange(NetIntAny):
    ...
    def emit_validate(self):
        return [
            f'if(!CheckInt("{self.name}", pObj->{self.name}, {self.min}, {self.max})) return -1;'
        ]

    def emit_unpack_check(self):
        return [
            f'if(!CheckInt("{self.name}", pMsg->{self.name}, {self.min}, {self.max})) break;'
        ]
...
class NetFlag(NetIntAny):
    ...
    def emit_validate(self):
        return [
            f'if(!CheckFlag("{self.name}", pObj->{self.name}, {self.mask})) return -1;'
        ]
