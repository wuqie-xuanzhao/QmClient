...
def hash_recursively(root):
    return [
        (relative, hash_file(absolute))
        for relative, absolute in collect_file_paths(root)
    ]
...
def compare_multiple(lst, data):
    total_download = set()
    total_remove = set()

    if len(lst) > 1:
        lst = tqdm(lst, leave=False)
...
""",
    ...
    p.add_argument(
        "version1", metavar="VER1", help="Original version to compare against"
    )
    ...
    pairs = [
        ...
        (
            f"QmClient-{args.version1}-linux_x86_64",
            f"QmClient-{args.version2}-linux_x86_64",
        ),
    ]
