...
def parse_arguments(arguments_line, num, name):
    try:
        ...
        parsed = next(
            csv.reader(
                io.StringIO(arguments_line), skipinitialspace=True, escapechar="\\"
            )
        )
        if len(parsed) != num:
            raise RuntimeError(
                f"Failed to parse {name} arguments: {arguments_line}, got {parsed}"
            )
        return parsed
    ...
    for line in lines:
        ...
        if args[1] == "STR":
            ...
            setting = {
                ...
                "default": parse_value(parsed[2]),
                "min": parse_value(parsed[3]),
                "max": parse_value(parsed[4]),
            }
        elif args[1] == "COL":
...
def export_commands(parsed_commands):
    ...
    for command in parsed_commands:
        output.append(
            f"  <tr><td>{html.escape(command['name'])}</td><td>{html.escape(command['arguments'])}</td><td>{html.escape(command['description'])}</td></tr>"
        )
...
def export_settings(parsed_settings):
    def export_color_value(setting):
        ...
        lightness = (number & 0xFF) / 255.0
        lightness = darkest_lgt + lightness * (1.0 - darkest_lgt)
        return f'<span class="colorpreview" style="background: hsla({h * 360.0:.3f}, {s * 100.0:.3f}%, {lightness * 100.0:.3f}%, {a:.3f})"></span> {number}'

    output = ['<div style="overflow-x: auto;"><table class="settingscommands">']
    output.append(
        "  <tr><th>Setting</th><th>Description</th><th>Default</th><th>Min</th><th>Max</th></tr>"
    )
    for setting in parsed_settings:
        ...
        if setting["type"] == "string":
            line = (
                line
                + f"<td>{html.escape(setting['default'])}</td><td></td><td></td></tr>"
            )
        elif setting["type"] == "int":
            line = (
                line
                + f"<td>{setting['default']}</td><td>{setting['min']}</td><td>{setting['max']}</td></tr>"
            )
        elif setting["type"] == "color":
            line = (
                line + f"<td>{export_color_value(setting)}</td><td></td><td></td></tr>"
            )
        else:
...
def export_tunings(parsed_tunings):
    ...
    for tuning in parsed_tunings:
        output.append(
            f"  <tr><td>{html.escape(tuning['name'])}</td><td>{html.escape(tuning['description'])}</td><td>{tuning['default']}</td></tr>"
        )
    output.append("</table></div>")
    return "\n".join(output)
...
def export_settings_commands_table(qm_only=False):
    print(
        "<!-- DO NOT EDIT MANUALLY! THIS FILE IS AUTOMATICALLY GENERATED BY qmclient_scripts/export_settings_commands_table.py -->"
    )
    ...
    if qm_only:
        ...
        export_block("QmClient Client Settings", export_settings(other_qm_settings))
        export_block("QmClient Voice Settings", export_settings(voice_settings))
        export_block("QmClient Commands", export_commands(qm_commands))
        return
    ...
    export_block(
        ...
        export_settings(
            [
                ...
                if "CFGFLAG_SERVER" in setting["flags"]
                and "CFGFLAG_GAME" in setting["flags"]
            ]
        )
        + "\n"
        + export_commands(
            [
                ...
                if "CFGFLAG_SERVER" in command["flags"]
                and "CFGFLAG_GAME" in command["flags"]
            ]
        ),
    )
    ...
    export_block(
        ...
        export_settings(
            [
                ...
                for setting in settings
                if "CFGFLAG_SERVER" in setting["flags"]
                and not is_qmclient_setting(setting)
            ]
        ),
    )
    ...
    export_block(
        ...
        export_commands(
            [
                ...
                for command in commands
                if "CFGFLAG_SERVER" in command["flags"]
                and "CFGFLAG_CHAT" not in command["flags"]
            ]
        ),
    )
    ...
    export_block(
        ...
        export_commands(
            [
                ...
                for command in commands
                if "CFGFLAG_SERVER" in command["flags"]
                and "CFGFLAG_CHAT" in command["flags"]
            ]
        ),
    )
    ...
    export_block(
        ...
        export_settings(
            [
                setting
                for setting in settings
                if "CFGFLAG_CLIENT" in setting["flags"]
                and not is_qmclient_setting(setting)
            ]
        ),
    )
    ...
    export_block(
        ...
        export_commands(
            [
                command
                for command in commands
                if "CFGFLAG_CLIENT" in command["flags"]
                and not is_qmclient_command(command)
            ]
        ),
    )
    ...
    qm_client_settings = [
        s
        for s in settings
        if "CFGFLAG_CLIENT" in s["flags"]
        and is_qmclient_setting(s)
        and not is_voice_setting(s)
    ]
    qm_voice_settings = [s for s in settings if is_voice_setting(s)]
    qm_client_commands = [
        c for c in commands if "CFGFLAG_CLIENT" in c["flags"] and is_qmclient_command(c)
    ]

    export_block("QmClient Client Settings", export_settings(qm_client_settings))

    export_block("QmClient Voice Settings", export_settings(qm_voice_settings))

    export_block("QmClient Commands", export_commands(qm_client_commands))

    export_block(
        "Tunings", export_tunings(parse_tunings(read_files("src/game/tuning.h")))
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export QmClient settings and commands table"
    )
    parser.add_argument(
        "--qm-only", action="store_true", help="Only export QmClient-specific settings"
    )
